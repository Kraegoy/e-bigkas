...
class VideoCallConsumer(AsyncWebsocketConsumer):
    output_text_f = ""
    cap = None  # Initialize cap as a class attribute
    recognizing = False  # Flag to indicate whether recognition is in progress
    stop_requested = False  # Flag to indicate if stop has been requested
    recognition_thread = None  # Thread to run the recognition loop
    recognized_actions = []
    ...
    def recognition_loop(self, sender_id, room_id):
        ...
        if cap.isOpened():
            sequence = []
            sentence = []  # Initialize sentence list
            ...
            with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.4) as holistic:
                while self.recognizing and cap.isOpened():
                    ...
                    if len(sequence) == 25:
                        ...
                        if(self.output_text_f != 'blank'):
                            self.output_text_f = " ".join(sentence)  # Assign output_text to instance variable
                            print(self.output_text_f)
                        ...
                        asyncio.run(self.send_frame(data_url, sender_id, room_id))
